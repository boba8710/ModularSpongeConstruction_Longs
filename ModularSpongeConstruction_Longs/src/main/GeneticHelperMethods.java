...
public class GeneticHelperMethods {
	 ...
	 */
	public long[] generateMersenneRandomString(int len, int seed) {
		MersenneTwister bustaRhymes = new MersenneTwister(seed);
		long[] output = new long[len];
		for(int i = 0; i < len; i++){
			output[i] = bustaRhymes.nextLong();
		}
		return output;
	}
	/**
	 ...
	 */
	public long[] generateMersenneRandomString(int len) {
	 ...
	 */
	public long[] flipRand(long[] in) {
		int randIndex = rand.nextInt(in.length);
		long[] temp = new long[in.length];
		for(int i = 0; i < in.length; i++){
			temp[i] = in[i];
		}
		int maskShift = rand.nextInt(64);
		long flippedLong = in[randIndex] ^(1L<<maskShift);
		temp[randIndex]=flippedLong;
		return temp;
	 ...
	 */
	public void runGenerationOnSortedPopulation(ModularSpongeConstruction_Longs[] population, double populationDieOffPercent, double mutationChance, double preserveTopNIndividuals) {
		ModularSpongeConstruction_Longs[] newPopulation = new ModularSpongeConstruction_Longs[population.length];
		ModularSpongeConstruction_Longs[] topIndividuals = new ModularSpongeConstruction_Longs[(int) (population.length-(population.length*(populationDieOffPercent)))]; //The population preserved will be 1-dieoff
		...
		for(int breedingIteration = 0 ; iterator < population.length; breedingIteration++) {
			for(int i = topIndividuals.length-2; i >= 0; i-=2) {
				...
				newPopulation[iterator] = new ModularSpongeConstruction_Longs(ModularSpongeConstruction_Longs.stateSize, ModularSpongeConstruction_Longs.rate, ModularSpongeConstruction_Longs.capacity, new ModularRoundFunction(ModularSpongeConstruction_Longs.stateSize, crossoverRoundFunction(topIndividuals[i].f.getFunc(),topIndividuals[(i+1+breedingIteration)%topIndividuals.length].f.getFunc(), mutationChance)));
				iterator++;
			}
		}
	...
	public void runAggressiveChangeGeneration(ModularSpongeConstruction_Longs[] population, double populationDieOffPercent) {
		...
		for(int i = newPopulation.length-2; i >= 0; i--) {
			newPopulation[i] = new ModularSpongeConstruction_Longs(ModularSpongeConstruction_Longs.stateSize, ModularSpongeConstruction_Longs.rate, ModularSpongeConstruction_Longs.capacity, new ModularRoundFunction(ModularSpongeConstruction_Longs.stateSize, mutateRoundFunction(topIndividual.f.getFunc(),0.50)));
		}
	 ...
	 */
	public void sortPopulationArray(ModularSpongeConstruction_Longs[] population) {
	 ...
	 * @param messageCount
	 * 
	 * This method updates the genetic and bitchange score associated with each individual, which are stored in the individual's ModularSpongeConstruction_Longs class. The scoring
	 ...
	 */
	public void score(ModularSpongeConstruction_Longs[] spongeArray, long[][] messages, long[][] messagesFlipped, int popSize, int messageCount) {
	 ...
	 */
	public void multithreadScore(ModularSpongeConstruction_Longs[] spongeArray, long[][] messages, long[][] messagesFlipped, int popSize, int messageCount) {
		final ExecutorService executor = Executors.newCachedThreadPool();
		final List<Future<?>> futures = new ArrayList<>();
		for(ModularSpongeConstruction_Longs sponge : spongeArray) {
